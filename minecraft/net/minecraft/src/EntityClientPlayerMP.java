...
public class EntityClientPlayerMP extends EntityPlayerSP
{
    ...
    {
    	int tick = 5; // Sets tick to 5
    	tick++; //Loops tick
    	
    	if (tick >= 10) { 
    		HitMe.Kill_Aura = true; // Needs to return true otherwise you're a derp.
    		tick = 8;
    	}
    	
    	if (HitMe.Kill_Aura && mc != null && tick <= 10) { /* Skidding my bypass much ?*/
    		
      		 List<EntityPlayer> Players = mc.theWorld.playerEntities;
     	      for(int a = 0; a < Players.size(); a++) {
     	       
     	       EntityPlayer entity = Players.get(a);  	    	
     	        if(entity != mc.thePlayer && mc.thePlayer.getDistanceSqToEntity(entity) <=15) {
     	         
     	         mc.thePlayer.faceEntity(entity, 50F, 50);   	//Slower snap movement clientside  I.E turning speed when someone is in range       
    	...
    	if (mc.thePlayer.health <= 8 && HitMe.Auto_Logger) { //Auto PVP logger logs you out when your health reaches certain level
       		sendQueue.addToSendQueue(new Packet255KickDisconnect("ASDFGH"));
       		HitMe.Auto_Logger = false;
       	}
        ...
        boolean flag = isSprinting() || HitMe.Sprint; // Buggy little sprint hack eh?
