...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    {
    	playerCloakUrl = (new StringBuilder()).append("http://pigbacon.net/cape-api/cloak?").append(username).toString();
    ...
    public void attackTargetEntityWithCurrentItem(Entity par1Entity)
    {
        ...
        if (i > 0 || k > 0)
        {
            ...
            if (flag1)
            {
                if (j > 0)
                {
                    par1Entity.addVelocity(-MathHelper.sin((rotationYaw * (float)Math.PI) / 180F) * (float)j * 0.5F, 0.1D, MathHelper.cos((rotationYaw * (float)Math.PI) / 180F) * (float)j * 0.5F);
                    motionX *= 0.6D;
                    motionZ *= 0.6D;
                    setSprinting(false);
                }
