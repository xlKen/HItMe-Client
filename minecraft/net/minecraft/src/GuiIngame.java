...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
            ...
            if (mc.playerController.shouldDrawHUD())
            {
                ...
                for (int k10 = 0; k10 < 10; k10++)
                {
                    if (l9 > 0)
                    {
                        ...
                        if (k10 * 2 + 1 < l9)
                        {
                            drawTexturedModalRect(j11, i9, 34, 9, 9, 9);
                        }

                        if (k10 * 2 + 1 == l9)
                        {
                            drawTexturedModalRect(j11, i9, 25, 9, 9, 9);
                        }

                        if (k10 * 2 + 1 > l9)
                        {
                            drawTexturedModalRect(j11, i9, 16, 9, 9, 9);
                        }
                    }
        ...
        if (mc.gameSettings.showDebugInfo)
        {
        	 ...
        	 * Check Keys here
        	 */
        	if (KeyCheck(Keyboard.KEY_X)) {
        		HitMe.Xray = !HitMe.Xray;
        		mc.renderGlobal.loadRenderers();
        	}
        	
        	if (KeyCheck(Keyboard.KEY_K)) {
        		HitMe.Kill_Aura = !HitMe.Kill_Aura;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_C)) {
        		HitMe.ChestESP = !HitMe.ChestESP;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_P)) {
        		HitMe.Tracers = !HitMe.Tracers;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_L)) {
        		HitMe.Auto_Logger = !HitMe.Auto_Logger;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_UP)) {
        		HitMe.Console = !HitMe.Console;
        		 mc.displayGuiScreen(new GuiConsole());
        	}
        	
        	if (KeyCheck(Keyboard.KEY_M)) {
        		HitMe.Dungeon_Master = !HitMe.Dungeon_Master;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_LCONTROL)) {
        		HitMe.Sprint = !HitMe.Sprint;
        	}
        	
        	if (KeyCheck(Keyboard.KEY_F9)) {
        		HitMe.Click_Gui = !HitMe.Click_Gui;
        		mc.displayGuiScreen(new GuiHitMeClick(mc));
        	}
        	
        	/**
        	 * Drawing stuff on screen now
        	 */
        	
        	drawString(fontrenderer, (new StringBuilder()).append("[X: ").append(Math.round(mc.thePlayer.posX)).append("]").toString(), 94, 4, 0x0066FF);
        	drawString(fontrenderer, (new StringBuilder()).append("[Y: ").append(Math.round(mc.thePlayer.posY)).append("]").toString(), 144, 4, 0x0066FF);
        	drawString(fontrenderer, (new StringBuilder()).append("[Z: ").append(Math.round(mc.thePlayer.posZ)).append("]").toString(), 184, 4, 0x0066FF);        	
    		newArialFont.drawStringS(this, "HitMe Client", 4, 4, 0x33FF00);
        	if (HitMe.Kill_Aura) {
        		newArialFont.drawStringS(this, "Kill Aura", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Kill Aura") - 2, m1, 0x00FFFF);
        		m1 += byte0;
        	}
        	
        	if (HitMe.Xray) {
        		newArialFont.drawStringS(this, "Xray", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Xray") - 2, m1, 0xFF0033);
        		m1 += byte0;
        	}
        	
        	if (HitMe.No_fall) {
        		newArialFont.drawStringS(this, "Nofall", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Nofall") - 2, m1, 0x66FF00);
        		m1 += byte0;
        	}
        	
        	if (HitMe.Tracers) {
        		newArialFont.drawStringS(this, "Tracers", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Tracers") - 2, m1, 0x3399FF);
        		m1 += byte0;
        	}
        	
        	if (HitMe.Auto_Logger) {
        		newArialFont.drawStringS(this, "PVP Logger", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("PVP Logger") - 2, m1, 0xFF0000);
        		m1 += byte0;
        	}
        	
        	if (HitMe.ChestESP) {
        		newArialFont.drawStringS(this, "ChestESP", scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("ChestESP") - 2, m1, 0xFF0000);
        		m1 += byte0;
        	}
        	
        	if (HitMe.Dungeon_Master) {
        		newArialFont.drawStringS(this, "Dungeon Master",scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Dungeon Master") - 2, m1, 0xFF0000);
        		m1 += byte0;
        	}
        	
        	if (HitMe.Sprint) {
        		newArialFont.drawStringS(this, "Sprint",scaledresolution.getScaledWidth() - fontrenderer.getStringWidth("Sprint") - 2, m1, 0x6633FF);
        		m1 += byte0;
        	}
